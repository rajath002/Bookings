...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	// if !ok {
	// 	m.App.Session.Put(r.Context(), "error", "can't get the reservation object from the session")
	// 	http.Redirect(w, r, "/", http.StatusSeeOther)
	// 	return
	// }
	...
	htmlMessage := fmt.Sprintf(`
		...
		Dear %s, <br/>
		This is to confirm your reservation from %s to %s made successfully.
...
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	...
	data := make(map[string]interface{})
	data["rooms"] = rooms // Attching Slice (Array) data to "data" map object
...
func (m *Repository) AvailabilityJSON(w http.ResponseWriter, r *http.Request) {
	...
	// Removed the error check since we handle all the aspects of the json right here
	out, _ := json.MarshalIndent(resp, "", "     ")
...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	...
	// Anytime when you login or logout make sure you renew the token
...
// AdminShowReservations show reservation in the admin tool
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	// Assume that there is no month and year specified
	now := time.Now()
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	form := forms.New(r.PostForm)
	for _, x := range rooms {
		// Get the block map from the session. if we have entry in the map
		// that does not exist in our posted data, and if the restriction id > 0, then it is a block
		// we need to remove
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int) // once we get the data the we cast it back to Map[string]int
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				...
				// the rest are just place holders for the days without blocks
				if val > 0 {
	...
	for name, _ := range r.PostForm {
		// log.Println("range has name ", name)
