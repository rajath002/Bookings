...
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	...
	_, err := m.DB.ExecContext(ctx, query, u.FirstName, u.LastName, u.LastName, u.Email, u.AccessLevel, time.Now())
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	query := `
		SELECT r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date, room_id, r.created_at, r.updated_at,
			rm.id, rm.room_name 
		FROM 
			reservations r 
		LEFT JOIN 
...
func (m *postgresDBRepo) InsertBlockFromRoom(id int, startDate time.Time) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
